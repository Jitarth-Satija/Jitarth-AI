...
st.markdown("""
    ...
    .gemini-logo { font-family: 'Google Sans', sans-serif; font-size: 32px; font-weight: 700; background: linear-gradient(to right, #4e7cfe, #f06e9c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-flex; align-items: center; white-space: nowrap; letter-spacing: -0.5px; }
    .logo-j { font-size: 44px; line-height: 0; margin-right: -2px; }
    .i-fix { font-size: 32px; font-weight: 700; } 
    .login-logo-container { text-align: center; margin-top: 50px; margin-bottom: 30px; display: flex; justify-content: center; }
    .login-logo-text { font-family: 'Google Sans', sans-serif; font-size: 48px; font-weight: 800; background: linear-gradient(to right, #4e7cfe, #f06e9c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-flex; align-items: center; letter-spacing: -1px; }
    ...
    .bday-timer { text-align: center; background: rgba(78, 124, 254, 0.1); border: 1px solid rgba(78, 124, 254, 0.3); border-radius: 8px; padding: 8px; margin: 10px 0; color: #4e7cfe; font-family: 'monospace'; }
    ...
    if st.session_state.get("forgot_mode"): recovery_ui(False)
        ...
        with tab2:
            ...
            if st.button("SIGN UP", use_container_width=True):
                ...
                elif nu != "" and len(nu_raw) >= 5:
                    if create_user(nu, st.session_state.reg_p, SECURITY_QUESTIONS.index(sq), sa):
        ...
        if p := st.chat_input("Ask Jitarth AI..."):
            ...
            with st.chat_message("assistant", avatar="✨"):
                try:
                    ...
                    - MASTER IDENTITY & RESPECT RULE:
                      1. The person logged in is "{current_user}".
                      2. If "{current_user}" == "Developer":
                         - This is your BOSS and CREATOR: Mast. Jitarth Satija.
                         - ALWAYS start responses with "Sir", "Boss", or "Master".
                         - Provide all info (Name, Birthday, Gender) together in one go.
                      3. If user is NOT "Developer":
                         - Treat as GUEST. Do NOT use "Sir/Boss".
                         - Follow 1-2-3 step disclosure (Name -> Birthday -> Gender).
                    ...
                    - NO KNOWLEDGE CUTOFF RULE:
                      - NEVER mention "knowledge cutoff" or "last update".
                      - If asked, simply say your knowledge is live and up-to-date as of {current_time_info}.
                      - Strictly prohibited to say "I don't have access to your birthday" to the Boss.

                    - DATABASE:
                      1. CREATOR (YOU): Mast. Jitarth Satija | Birthday: 30th January | Gender: Male
                      2. FATHER: Mr. Rajaram Satija | 4th Feb 1985 | Male
                      3. MOTHER: Mrs. Vartika Satija | 17th Sept 1984 | Female
                      4. BROTHER: Mast. Rudransh Satija | 16th Oct 2023 | Male
                      5. BEST FRIEND: Miss. Meet Gera | 30th Sept 2012 | Female

                    - CORE CONSTRAINTS:
                      - Never reveal the username "Developer".
