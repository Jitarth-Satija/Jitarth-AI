...
# 2. Ultra Deep Search Function
def search_internet(query):
    try:
        with DDGS() as ddgs:
            # Query ko 2026 ke liye super-optimize kiya
            deep_query = f"{query} latest news 2025 2026 report"
            
            # Text search is more reliable for specific data like % tariffs
            results = list(ddgs.text(deep_query, max_results=6))
            
            if results:
                context = "\n".join([f"Source: {r.get('title', '')} - {r.get('body', '')}" for r in results])
                return context
            return "No real-time data found in search."
    except Exception:
        return "Search Service Busy."
...
if st.session_state.logged_in_user is None:
    ...
    if st.session_state.get("forgot_mode"): 
        ...
        with tab2:
            ...
            nu_raw = st.text_input("Choose Username (5-20)", value=nu_val, key="reg_u")
            ...
            for i, s in enumerate(suggs):
                if cols[i].button(s, key=f"sug_reg_{s}", use_container_width=True):
                    ...
                    st.rerun()
            np_raw = st.text_input("Create Password (4-10)", type="password")
            np = validate_password(np_raw)
            sq = st.selectbox("Security Question", SECURITY_QUESTIONS)
            sa = st.text_input("Answer")
            if st.button("SIGN UP", use_container_width=True):
                if create_user(nu, np, SECURITY_QUESTIONS.index(sq), sa): st.success("Created!"); st.session_state.suggested_un = ""
    ...
    if st.session_state.show_settings:
        if st.session_state.get("settings_recover_mode"): recovery_ui(True)
        else:
            st.title("⚙️ Settings")
            v_p = st.text_input("Unlock Settings with Password", type="password")
            if v_p == user_record[1]:
                with st.expander("👤 Update Profile"):
                    ...
                    ua = st.text_input("Answer", value=user_record[4])
                    if st.button("Save Changes"): confirm_dialog("Update details?", "update_profile", (current_user, nu_settings, np_settings, SECURITY_QUESTIONS.index(uq), ua))
                with st.expander("⚠️ Danger Zone"):
                    if st.button("🔴 Logout Account"): confirm_dialog("Logout?", "logout")
                    if st.button("🗑️ Delete History"): confirm_dialog("Clear chats?", "delete_chats")
                    if st.button("❌ Delete Account"): confirm_dialog("Delete?", "delete_account")
    else:
        # Chat Logic
        messages = st.session_state.temp_messages if st.session_state.is_temp_mode else (user_chats.get(st.session_state.get("current_session", ""), []) if st.session_state.get("current_session") else [])
        
        if not messages:
            if st.session_state.is_temp_mode: st.markdown('<div class="temp-warning">🔒 Temporary Chat</div>', unsafe_allow_html=True)
            else: st.markdown(f"<h2 style='text-align:center; margin-top:100px;'>Hello, {current_user}</h2>", unsafe_allow_html=True)
        ...
        if p := st.chat_input("Ask ✨Jitarth AI..."):
            ...
            with st.chat_message("assistant", avatar="✨"):
                with st.status("🔍 Searching Web (2025-2026)...", expanded=False): 
                    internet_context = search_internet(p)
                
                # FORCE AI TO USE SEARCH DATA
                sys_prompt = f"""You are ✨Jitarth AI, created by Jitarth Satija. Today is January 30, 2026.
                FORGET YOUR OLD 2025 CUTOFF. You must only answer using the provided SEARCH DATA.
                
                SEARCH DATA FOR THIS QUERY:
                {internet_context}
                
                If the Search Data contains information about Trump's tariffs or the Pahalgam attack, explain it in detail. 
                Even if search data is thin, look for keywords like '%' or 'Trump' and explain. 
                DO NOT say you don't know if search data is provided. Answer in the user's language."""
                ...
                try:
                    ...
                    st.rerun()
                except: st.error("Server error. Try again.")
